...
class smo_svm():
        ...
        while (i < maxiter):
            ...
            for _ in range(m):
                ...
                if ((mat_y[_] * E_ < -toler) and (alphas[_] < C)) or ((mat_y[_] * E_ > toler) and (alphas[_] > 0)):
                    ...
                    alphas[j] = smo_svm.clipAlpha(alphas[j], H, L)
    ...
    def smo_simple(x, y, C = 1, toler = 0.01, maxiter = 30):
        if 0 in y:
            y = y.map({0 : -1, 1 : 1})
        b, alphas = smo_svm.smo_simple_core(x, y, C, toler, maxiter)
        w = smo_svm.cal_w(alphas, x, y)
        return b, alphas, w 

    def evaluation(pred_y, y):
        if type(y) != list:
            y = list(y)
        acc = smo_svm.svm_accuracy(pred_y, y)
        print("accuracy: {}\n\nconfusion matrix:".format(acc))
        tn, fn, fp, tp = smo_svm.svm_confusion_mat(pred_y, y)
